...
export const MobileJourneyView: React.FC = () => {
  ...
  const [geocodeDestError, setGeocodeDestError] = useState<string | null>(null);
  ...
  // Use live GPS location for origin with optional reverse geocoding
  const handleUseLiveGpsOrigin = async () => {
    setIsLocatingOrigin(true);
    refreshLiveGps();
    setOrigin(liveCoords.locationName || `Live GPS (${liveCoords.lat.toFixed(4)}°N, ${liveCoords.lng.toFixed(4)}°E)`);
    try {
      const addr = await reverseGeocode(liveCoords.lat, liveCoords.lng);
      if (addr) {
        setOrigin(addr.split(',').slice(0, 2).join(',').trim());
      }
    } catch {
      // Keep existing formatted GPS
    } finally {
      setIsLocatingOrigin(false);
    }
  };

  // Lookup and pin destination on the map via Nominatim geocoder
  const handleLookupDestination = async (overrideText?: string) => {
    const textToSearch = (overrideText || destination).trim();
    if (!textToSearch) return;
    setIsGeocodingDest(true);
    setGeocodeDestError(null);
    try {
      const res = await geocodeAddress(textToSearch);
      if (res) {
        setMapDestCoords({ lat: res.lat, lng: res.lng });
        setMapDestName(res.displayName);
        const shortName = res.displayName.split(',').slice(0, 2).join(',').trim();
        setDestination(shortName);
      } else {
        setGeocodeDestError('Place not found. Try adding city/area.');
        setTimeout(() => setGeocodeDestError(null), 3500);
      }
    } catch {
      setGeocodeDestError('Map geocoding service unavailable.');
      setTimeout(() => setGeocodeDestError(null), 3500);
    } finally {
          ...
          <div className="space-y-3 pt-2">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <button
                  type="button"
                  onClick={handleUseLiveGpsOrigin}
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-semibold text-slate-400">Target Destination</label>
                {mapDestCoords && (
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="e.g., Home (West Oak Ave) or address & Enter"
                  className="w-full pl-3.5 pr-20 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 transition-all"
                />
                <button
                  type="button"
                  onClick={() => handleLookupDestination()}
                  disabled={isGeocodingDest || !destination.trim()}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 px-2.5 py-1.5 bg-rose-600/80 hover:bg-rose-500 text-white rounded-lg text-xs font-semibold flex items-center gap-1 transition-all disabled:opacity-40"
                  title="Search & pin destination on map"
                >
                  {isGeocodingDest ? (
